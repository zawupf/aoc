...
let parseReport (line: string) : Report = line.Split [| ' ' |] |> Array.map int
...
let generateSubsets (report: Report) =
    seq {
...
let part2 input =
    input
    |> Array.sumBy (fun line ->
        if line |> parseReport |> isGraduallyChangingWithProblemDampener then
            1
        else
            0)
