...
namespace Aoc._2019._07.Tests
{
    public class Run_Should
    {
        ...
        public void MaxThrusterSignal_Works(int expected, string code)
        {
            var result = Run.MaxThrusterSignal(_05.Computer.Compile(code));
        ...
        public void MaxThrusterSignalFeedback_Works(int expected, string code)
        {
            var result = Run.MaxThrusterSignalFeedback(_05.Computer.Compile(code));
            Assert.Equal(expected, result);
        }
        ...
        {
            var run = new Run();
            Assert.Equal("11828", run.Job1());
            Assert.Equal("1714298", run.Job2());
        }
    }
}
