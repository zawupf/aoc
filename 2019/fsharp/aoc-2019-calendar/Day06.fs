...
let minimalTransferCount orbits =
    ...
    (List.except planets1 planets2 |> List.length)
    + (List.except planets2 planets1 |> List.length)

let job1 () =
    readInputLines "06" |> parse |> checksum |> string

let job2 () =
    readInputLines "06" |> parse |> minimalTransferCount |> string
