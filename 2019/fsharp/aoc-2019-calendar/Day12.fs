...
module Moon =
    ...
    let private updateVelocity moons moon =
        let { pos = Pos(x, y, z); vel = Vel(vx, vy, vz) } = moon
        let (Vel(dx, dy, dz)) =
            moons
            |> Seq.fold (fun (Vel(vx, vy, vz)) { pos = Pos(mx, my, mz) } ->
                Vel(vx + compare mx x, vy + compare my y, vz + compare mz z)) (Vel(0, 0, 0))
...
let job2() =
    ...
    let x =
        moons
        |> List.map (Moon.only 'x')
        |> Moon.cycleCount

    let y =
        moons
        |> List.map (Moon.only 'y')
        |> Moon.cycleCount

    let z =
        moons
        |> List.map (Moon.only 'z')
        |> Moon.cycleCount
