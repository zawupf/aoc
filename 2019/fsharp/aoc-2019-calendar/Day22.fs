...
let cut n cards =
    let m = if n > 0 then n else (cards |> Array.length) + n
    (cards |> Array.skip m, cards |> Array.take m) ||> Array.append

let beforeCut n length index =
    let m = if n > 0 then n else length + n
...
let (|DealIntoNewStack|Cut|DealWithIncrement|) (line: string) =
    if line = "deal into new stack" then
        DealIntoNewStack
    else if line.StartsWith("cut ") then
        Cut(line.Substring(4) |> int)
    else if line.StartsWith("deal with increment ") then
        DealWithIncrement(line.Substring(20) |> int)
    else
...
let shuffle n lines =
    ...
    |> Seq.fold
        (fun cards ->
            ...
            | Cut n -> cut n cards
            | DealWithIncrement n -> dealWithIncrement n cards)
        (deck n)
