...
module Reaction =
    let private parseInput (string: string) =
        string.Split(',')
        |> Array.map (String.trim >> Chemical.parse)
        |> Array.toList

    let private parseOutput (string: string) = string |> (String.trim >> Chemical.parse)
...
module Factory =
    ...
    let rec private require (Chemical(_, name) as chemical) factory =
        let produce (Chemical(count, name)) factory =
            ...
            | _ ->
                ...
                let num =
                    count / outputCount + match count % outputCount with
                                          | 0L -> 0L
                                          | _ -> 1L

                reaction
                |> Reaction.inputs
                |> List.fold (fun factory (Chemical(count, name)) ->
                    factory
                    |> require (Chemical(num * count, name))
                    |> sub (Chemical(num * count, name))) factory
    ...
    let rec private fuelRange availableOre min factory =
        ...
        if ore <= availableOre then factory |> fuelRange availableOre max else min, max
