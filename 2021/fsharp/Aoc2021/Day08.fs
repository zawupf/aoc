...
let decodeSum lines =
    ...
    |> List.map (fun line ->
        let wrongDigits =
            ...
            |> String.split ' '
            |> Array.take 10
            |> Array.map sortSegments

        let wrong1 =
            wrongDigits
            |> Array.find (fun digit -> digit.Length = 2)
            |> List.ofSeq

        let wrong4 =
            wrongDigits
            |> Array.find (fun digit -> digit.Length = 4)
            |> List.ofSeq

        let wrong7 =
            wrongDigits
            |> Array.find (fun digit -> digit.Length = 3)
            |> List.ofSeq

        let wrong8 =
            wrongDigits
            |> Array.find (fun digit -> digit.Length = 7)
            |> List.ofSeq
        ...
        seq {
            ...
            for cf in cfPerms do
                ...
                let bdPerms =
                    wrong4 |> List.except wrong1 |> permutations
                ...
                for bd in bdPerms do
                    ...
                    let a =
                        wrong7 |> List.except wrong4 |> List.head
                    ...
                    for eg in egPerms do
                        ...
                        if digits
                           |> List.forall (fun digit -> decodedDigits |> Array.contains digit) then
                            yield
                                line
                                |> String.split ' '
                                |> Array.skip 11
                                |> Array.map (fun digit ->
                                    ...
                                    let number =
                                        digits
                                        |> List.findIndex (fun d -> d = decodedDigit)
