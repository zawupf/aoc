...
let findPositionWithMinFuel distanceToFuel positions =
    let positionCount = positions |> List.countBy (fun position -> position)
    ...
    [ min..max ] |> List.map (fun pos -> pos, pos |> fuel) |> List.minBy snd
...
let input = "07" |> readInputText |> String.parseInts ','

let job1 () =
    input |> findBestPositionWithMinFuel |> snd |> string

let job2 () =
    input |> findBestCrabPositionWithMinFuel |> snd |> string
